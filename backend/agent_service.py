        ...
        for round_idx in range(self.MAX_ROUNDS):
            ...
            for result in results:
                if isinstance(result, str) and result.startswith("__ASK_USER__:"):
                    question = result.replace("__ASK_USER__:", "").strip()
                    # 把追问存入记忆（role=assistant），让下轮对话知道问过什么
                    memory.add({"role": "assistant", "content": question})
                    # 流式推送追问给前端，用特殊前缀区分
                    yield f"__ASK_USER__:{question}"
                    return  # 终止循环，等用户回答

            # 没有 ask_user，正常把工具结果追加到历史
